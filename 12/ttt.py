...
class Game():
  def __init__(self, game_id, name):
    self.avaiable_slots = 1 # 2 players the first player is inplicit when game is created
    self.id = game_id
    self.name = name
    self.board = [
      [ 0, 0, 0 ],
...
class GamesManager():
  ...
  def connect_to_game(self, game_id):
    self.get_game(game_id).connect_to_game()
    ...
    return [
      {
        'name': game.name,
        'id': game_id
      } for game_id, game in self.games.items() if game.avaiable_slots != 0
    ]
...
class Handler(BaseHTTPRequestHandler):
  ...
  def handle_list(self):
    self.send_result(200, gameManager.get_games_list())
